...
*/
	...
	public ResultUtil login(HttpServletRequest req, String username, String password, String vcode) {
		...
		try{
			...
			subject.login(token);
			/*String tok = Double.toString(Math.random());
			tok.substring(0,10);
			TbAdmin admin = adminServiceImpl.login(username, password);
			admin.setToken(tok.substring(0,10));
			adminServiceImpl.updateToken(admin);
...
//		TbAdmin admin=(TbAdmin) req.getSession().getAttribute("admin");
	...
	public String logList(HttpServletRequest req,HttpServletResponse response) throws IOException {
		/*String xx = req.getSession().getAttribute("cstoken").toString();
		String username=req.getSession().getAttribute("username").toString();
		String password=req.getSession().getAttribute("password").toString();
		TbAdmin admin = adminServiceImpl.login(username, password);
		if(xx.equals(admin.getToken())){
			return "page/log/logList";
		}else{

			return "http://localhost:8080/login.jsp";
		}*/
		return "page/log/logList";
	}
