 ...
 * 
 * 该实例演示如何对 Ping++ webhooks 通知进行验证。 验证是为了让开发者确认该通知来自 Ping++
 * ，防止恶意伪造通知。用户如果有别的验证机制，可以不进行验证签名。
 * 
 * 验证签名需要 签名、公钥、验证信息，该实例采用文件存储方式进行演示。 实际项目中，需要用户从异步通知的 HTTP header 中读取签名，从 HTTP
 * body 中读取验证信息。公钥的存储方式也需要用户自行设定。
 * 
...
public class WebhooksVerify {
	...
	public static boolean runDemos(String webhooksRawPostData) throws Exception {
